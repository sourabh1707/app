...
public function index(){
   ...
   public function get_details(){
        ...
        $page_data['page_title'] = 'User Details';
        $id = $this->input->post('id');
        //   print_r($id); exit();
        $id = decode_string($id);
        $client = $this->CRUD->get_data_row(TBL_USER,array('id'=>$id));
        if($client){
        // print_r($client); exit();
        $json['status'] = 200;
        $json['message'] = $this->db->get_where(TBL_ERROR_CODE,array('code'=>$json['status']))->row()->msg;
        $json['details1'] = $this->load->view('log/client/template-user',array('user'=>$client),true);
