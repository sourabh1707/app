        ...
        if($type=='change_status'){
            ...
            if($action!=''){
                if($action=='change_status'){
                    ...
                    $upd_id = $this->CRUD->update_data(TBL_SCHEDULE,$up_data,array('id'=>$id));
                    if($upd_id){
                        if($status=='true'){
                            $json['status'] = 200;
                            $json['message'] = $this->db->get_where(TBL_ERROR_CODE,array('code'=>$json['status']))->row()->msg;
        ...
        if($type=='details'){
            ...
            if($action!=''){
                if($action=='get_details'){
                    $id = $this->input->post('id');
                    $id = decode_string($id);
                    $schedule = $this->CRUD->get_data_row(TBL_SCHEDULE,array('id'=>$id));
                    if($schedule){
                        $json['status'] = 200;
                        $json['message'] = $this->db->get_where(TBL_ERROR_CODE,array('code'=>$json['status']))->row()->msg;
        ...
        if($type=='delete'){
            ...
            if($action!=''){
                if($action=='delete_details'){
                    ...
                    $delete = $this->CRUD->update_data(TBL_SCHEDULE,$schedule,array('id'=>$id));
                    if($delete){
                        $json['status'] = 200;
                        $json['message'] = $this->db->get_where(TBL_ERROR_CODE,array('code'=>$json['status']))->row()->msg;
                        $json['details'] = $this->load->view($this->module_name.'/template-details',array('schedule'=>$schedule),true);
                        ...
                        $log_data = array();
                                ...
                                $log_data['message'] = 'schedule_remove_successfully';
                               schedule_log($log_data,$admin_id,$id,0);
                    }
                }
            }
        }
