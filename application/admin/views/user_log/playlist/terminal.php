...
<div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel"> 
          <div class="x_content">
           ...
           // print_r($tid);exit();
             ...
             $tt1=$this->db->query("SELECT a.`groupt_id`, a.`created_by` FROM `tbl_playlist_log` a,`tbl_playlist` b, `tbl_group_terminal` c WHERE a.`playlist_id` = b.id AND a.groupt_id =c.id AND a.`playlist_id`=$pid AND a.id=$tid");

           //  echo "SELECT a.`groupt_id` FROM `tbl_playlist_log` a,`tbl_playlist` b, `tbl_group_terminal` c WHERE a.`playlist_id` = b.id AND a.`created_by`=1 AND a.groupt_id =c.id AND a.`playlist_id`=$pid AND a.id=$tid";
        ...
        $playlist2 = $this->db->query("SELECT a.`terminal_id`, a.`created_by` FROM `tbl_playlist_log` a,`tbl_playlist` b, `tbl_terminal` c WHERE a.`playlist_id` = b.id AND a.terminal_id =c.id AND a.`playlist_id`=$pid AND a.id=$tid");

         $playlist4 = $this->db->query("SELECT a.id as tid, a.terminal_id, a.groupt_id, a.`created_by` FROM `tbl_playlist_log` a WHERE a.`playlist_id`=$pid AND a.id=$tid AND a.terminal_id=0 AND a.groupt_id=0");
...
$playlist1 = $this->db->query("SELECT a.id as tid,c.name, c.id as gid, a.`created_by` FROM `tbl_playlist_log` a, `tbl_group_terminal` c WHERE a.`playlist_id`=$pid AND a.id=$tid AND c.id IN($grp_terminal)"); 

 //echo "SELECT a.id as tid, b.id,c.name, c.id, a.`groupt_id` FROM `tbl_playlist_log` a,`tbl_playlist` b, `tbl_group_terminal` c WHERE a.`playlist_id` = b.id AND a.`created_by`=$user_id AND a.groupt_id =c.id AND b.is_delete !=1 AND a.`playlist_id`=$pid AND a.groupt_id IN($grp_terminal) AND a.id=$tid";
//echo "SELECT a.id as tid,c.name, c.id as gid FROM `tbl_playlist_log` a, `tbl_group_terminal` c WHERE a.`created_by`=1 AND a.`playlist_id`=$pid AND a.id=$tid AND c.id IN($grp_terminal) AND a.`created_by`=$user_id";
   ...
   //print_r($terminal1); exit();
                   ...
                   <td><input type="button" name="view" value="<?php echo $name;?>" id="<?php echo $id; ?>" class="btn btn-info btn-xs view_data" /></td>
                        ...
                        <td><a href="<?php echo site_url("playlist_log/pdf/".$terminal1->tid."/$id");?>" type="button" target="_blank" class="btn btn-primary" /><i class="fa fa-file-pdf-o"></i></a> </td>
     ...
     if (isset($playlist2) && !empty($playlist2)) {
          ...
          $playlist3 = $this->db->query("SELECT a.id as tid,c.name, c.id as gid, a.`created_by` FROM `tbl_playlist_log` a, `tbl_terminal` c WHERE a.`playlist_id`=$pid AND a.id=$tid AND c.id =$terminal");
         ...
         foreach ($playlist3->result() as $terminal3){
                   ...
                   <td><input type="button" name="view" value="<?php echo $name;?>" id="<?php echo $id; ?>" class="btn btn-info btn-xs view_data1" /></td>
                         ...
                         <td><?php echo $this->CRUD->get_data_row(TBL_USER,array('id'=>$uid))->name; ?></td>
                        <td><a href="<?php echo site_url("playlist_log/pdf/".$terminal3->tid."/$id");?>" type="button" target="_blank" class="btn btn-primary" /><i class="fa fa-file-pdf-o"></i></a> </td>
                    </tr>
          ...
          // echo $tt=$terminal4->tid; exit();?>                            
                    ...
                    <td>-</td>
                      ...
                      <td><?php echo $this->CRUD->get_data_row(TBL_USER,array('id'=>$uid))->name; ?></td>
                        ...
                        <td><a href="<?php echo site_url("playlist_log/pdf/".$tid);?>" type="button" target="_blank" class="btn btn-primary" /><i class="fa fa-file-pdf-o"></i></a> </td>        
                    </tr>
                    <?php       
      }  }else {
                ?>
...
<script>  
 $(document).ready(function(){  
      $('.view_data').click(function(){  
           var id = $(this).attr("id");  
           $.ajax({  
                url:"<?php echo base_url(); ?>admin.php/playlist_log/get_details",  
                method:"post",  
                data:{id:id},  
 ...
 $(document).ready(function(){  
      $('.view_data1').click(function(){  
           var id = $(this).attr("id");  
           $.ajax({  
                url:"<?php echo base_url(); ?>admin.php/playlist_log/get_details1",  
                method:"post",  
                data:{id:id},  
                success:function(data){  
                     $('#terminal_detail').html(data);  
