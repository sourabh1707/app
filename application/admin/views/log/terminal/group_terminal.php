...
<link href="<?php echo base_url('template/admin') ?>/vendors/bootstrap-daterangepicker/daterangepicker.css" rel="stylesheet">
 ...
 <!-- Datatable Dependency start -->
  ...
  <!-- Search filter -->
  <center>
   <form method='post' action='<?php echo site_url(); ?>group_terminal_log/fetch_terminal_data'>
    ...
    <table class="table table-striped table-bordered" id="table_id">
      <thead>
        <tr>
          ...
          <th class="header">Group Terminal Name</th>
          <th class="header">Terminal Status</th>
         <!-- <th class="header">Duration(Second)</th> -->
         <th class="header">Group Assign To</th>
       ...
       <tbody>
            ...
            if (isset($terminalInfo) && !empty($terminalInfo)) {
                foreach ($terminalInfo as $key => $element) {
                    //print_r($element);die();
                  //print_r($element['playlist']);die();
                    ...
                    <tr>
                        ...
                        <td><?php echo $this->CRUD->get_data_row(TBL_USER,array('id'=>$element['user_id']))->name; ?></td> 
                         <td><?php echo translate($rvalue);?></td> 
                         <td><?php  $d1=$element['log_created_on'];
                            echo $date_on=date("Y/m/d",strtotime($d1));
                         ?></td>

                         <td><input type="button" name="view" value="<?php echo $element['name'];?>" id="<?php echo $element['groupt_id']; ?>" class="btn btn-link view_data" readonly/></td> 
...
<div class="modal-content" style="width: 880px;">           
    <div class="modal-header" style="color: black;">  
                     ...
                     <h4 class="modal-title">Group Terminal Details</h4>  
                </div>  
      ...
      $('.view_data').click(function(){  
           ...
           $.ajax({  
               // url:"<?php echo base_url(); ?>Layout_log/get_layout_details",
                url:"<?php echo base_url(); ?>admin.php/group_terminal_log/get_details",  
                method:"post",  
                data:{id:id},  
