...
<div class="row">
  ...
  <!--Schedule Playlist-->                
    <table id="list" class="table table-striped table-bordered">
        ...
        <tbody>
            <?php
             $admin_id = $this->session->userdata('admin_id');
              ...
              $playlist = $this->db->query("SELECT a.id,b.name,a.schedulep_id, b.schedule_on, b.schedule_to FROM `tbl_admin_schedule_log` a, tbl_schedule b WHERE a.schedulep_id=b.id AND b.is_deleted !=1 AND a.created_by=$admin_id");
        ...
        <td><a href="<?php echo site_url("admin_schedule_log/pdf/".$row->id);?>" type="button" target="_blank" class="btn btn-primary btn-sm" /><i class="fa fa-file-pdf-o"></i></a> </td>
...
<!--Schedule Layout-->
        ...
        <tbody>
            <?php
             $admin_id = $this->session->userdata('admin_id');
              $playlist = $this->db->query("SELECT a.id,b.name,a.schedulel_id, b.schedule_on, b.schedule_to FROM `tbl_admin_schedule_log` a, tbl_schedule_l b WHERE a.schedulel_id=b.id AND b.is_deleted !=1 AND a.created_by=$admin_id");
...
<script>  
 $(document).ready(function(){  
      $('.view_data').click(function(){  
           var id = $(this).attr("id");  
           $.ajax({  
                url:"<?php echo base_url(); ?>admin.php/admin_schedule_log/get_playlist_details",  
                method:"post",  
                data:{id:id},  
...
<script>  
 $(document).ready(function(){  
      $('.view_data1').click(function(){  
        ...
        //  alert(id); 
           $.ajax({  
                url:"<?php echo base_url(); ?>admin.php/admin_schedule_log/get_playlist_details1",  
                method:"post",  
                data:{id:id},  
                success:function(data){  
