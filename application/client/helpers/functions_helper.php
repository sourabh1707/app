...
if(!function_exists('add_log1')){
    function add_log1($log_data,$user='',$gp_id=0,$playlist_id,$starttime,$endtime,$hourdiff,$ter_id){
        unset($log_data['response']);
        unset($log_data['status']);
       ...
       // $data['from'] = $from;
        ...
        $CI->load->database();
        $CI->db->insert(TBL_PLAYLIST_LOG,$data);
    }
}

if(!function_exists('add_log2')){
    function add_log2($log_data,$user='',$schedulep_id,$schedulel_id){
        unset($log_data['response']);
        unset($log_data['status']);
       ...
       // $data['from'] = $from;
        ...
        $CI->load->database();
        $CI->db->insert(tbl_schedule_log,$data);
    }
}

if(!function_exists('add_log3')){
    function add_log3($log_data,$user='',$gp_id=0,$layout_id,$starttime,$endtime,$hourdiff,$ter_id){
        unset($log_data['response']);
        unset($log_data['status']);
       ...
       // $data['from'] = $from;
        ...
        $CI->load->database();
        $CI->db->insert(TBL_LAYOUT_LOG,$data);
    }
}


if(!function_exists('door_log')){
    function door_log($user='',$terminal_id,$status,$open,$close,$onoff){
        //unset($log_data1['response']);
       // unset($log_data1['status']);
        ...
        //$data['message'] = serialize($log_data1);
        $data['terminal_id'] = $terminal_id;
        $data['created_by'] = $user;
        $data['opentime'] = $open;
        $data['closetime'] = $close;
        $data['log_created_on'] = date(DB_DATETIME_FORMAT);
        
        $CI->load->database();
        $CI->db->insert(TBL_ALARM_LOG,$data);
    }
}
