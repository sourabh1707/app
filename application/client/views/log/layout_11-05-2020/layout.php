...
<div class="row">
    ...
    <table id="list" class="table table-striped table-bordered">
       ...
       $layout=$this->db->select('name')
          ...
          $layout = $this->db->query("SELECT t2.id, t2.name, t2.is_active, t1.duration, t1.id as tid, t2.is_delete FROM tbl_layout_log t1 JOIN tbl_layout t2 ON t2.id = t1.layout_id AND t1.created_by=1");
             //$layout = $this->db->query("SELECT DISTINCT(t2.id), t2.name, SUM(t1.duration) as duration, t1.id as tid, t2.is_delete FROM tbl_layout_log t1 JOIN tbl_layout t2 ON t2.id = t1.layout_id AND t1.created_by=1 AND t2.is_delete !=1 GROUP BY t2.id");

            // echo "SELECT t2.id, t2.name, t2.is_active, t1.duration, t1.id as tid, t2.is_delete FROM tbl_layout_log t1 JOIN tbl_layout t2 ON t2.id = t1.layout_id AND t1.created_by=1 AND t2.is_delete !=1";

           //  $layout = $this->db->query("SELECT DISTINCT(b.id),b.name, a.`created_by` FROM `tbl_layout_log` a,`tbl_layout` b WHERE a.`layout_id` = b.id AND a.`created_by`=$user_id");

            $i = 1;
            if (isset($layout) && !empty($layout)) {
                foreach ($layout->result() as $row) {
                    $id= $row->id;
                    ?>
                    <tr>
                       <td><?php echo $i++; ?></td> 
           ...
           <!-- <a href="<?php echo site_url()?>billing/terminal_list/<?php echo $row->id?>" class="btn btn-primary"><i class="fa fa-eye"></i></a>-->
            <a href="<?php echo site_url()?>layout_log/terminal_list/<?php echo $row->id."/".$row->tid?>" class="btn btn-info btn-sm">Terminal</a> 
...
<div class="modal-content" style="width: 980px; margin-left: -100px;">           
...
<script>  
 $(document).ready(function(){  
      $('.view_data').click(function(){  
           var id = $(this).attr("id");  
           $.ajax({  
                url:"<?php echo base_url(); ?>layout_log/get_layout_details",  
                method:"post",  
                data:{id:id},  
