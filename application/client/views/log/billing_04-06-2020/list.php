...
<form method="POST" action="" name="ofrm">
    <table id="list" class="table table-striped table-bordered">                       
        ...
        <tbody>
            <?php
             ...
             $terminal = $this->db->query("SELECT t2.id, t2.name,t1.opentime, t1.closetime, t1.status, t1.id as tid FROM tbl_alarm_log t1 JOIN tbl_terminal t2 ON t1.terminal_id = t2.name AND t1.created_by=$user_id AND t1.status='offline'");

           // echo "SELECT t2.id, t2.name,t1.opentime, t1.closetime, t1.status, t1.id as tid FROM tbl_alarm_log t1 JOIN tbl_terminal t2 ON t1.terminal_id = t2.name AND t1.created_by=$user_id AND t1.status='offline'";
    ...
    //$playlist = $this->db->query("SELECT t2.id, t2.name, t1.duration, t1.id as tid, t2.is_deleted,t1.schedule_on,t1.schedule_to FROM tbl_playlist_log t1 JOIN tbl_schedule t2 ON t2.playlist_id = t1.playlist_id AND t1.created_by=1 AND t1.schedule_on = t2.schedule_on AND t1.schedule_to = t2.schedule_to AND t1.terminal_id=$ter_id");

       $playlist = $this->db->query("SELECT t2.id, t2.name, t2.is_active, t1.duration, t1.terminal_id, t1.id as tid, t2.is_delete,t1.schedule_on,t1.schedule_to FROM tbl_playlist_log t1 JOIN tbl_playlist t2 ON t2.id = t1.playlist_id AND t1.created_by=$user_id AND t1.terminal_id=$ter_id");

      //echo "SELECT t2.id, t2.name, t2.is_active, t1.duration, t1.terminal_id, t1.id as tid, t2.is_delete,t1.schedule_on,t1.schedule_to FROM tbl_playlist_log t1 JOIN tbl_playlist t2 ON t2.id = t1.playlist_id AND t1.created_by=$user_id AND t1.terminal_id=$ter_id GROUP BY t2.id";  

             //echo "SELECT t1.duration, t1.id as tid,t1.schedule_on,t1.schedule_to FROM tbl_playlist_log t1 WHERE t1.created_by=$user_id AND t1.terminal_id=$ter_id";
             foreach ($playlist->result() as $row1) {

                      $playlist_name=$row1->name;
                      $playlist_id=$row1->id;
                      $terid= $row1->terminal_id;
                  ...
                  $hours1 = floor(($diff1 - $years1 * 365*60*60*24 - $months1*30*60*60*24 - $days1*60*60*24)
                    / (60*60));  
                ...
                ?>
                  ...
                  <!--  <td><input type="text" name="num1" id="num1" value="" style="width: 45px;"><strong>*</strong><input type="text" name="num2" id="num2" value="" style="width: 45px;"><input type="button" onClick="multiplyBy()" Value="Multiply" name="res" class="btn" />Result:<span id = "result" name="result"></span> 
                   ...
                   <td><a href="<?php echo site_url("billing_log/cal/".$tid."/".$playlist_id."/".$timediff."/".$ter_id);?>" type="button" target="_blank" class="btn btn-warning btn-sm" />Bill</a> </td>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body" id="terminal_detail1">
            ...
            <input type="text" name="terminal_id" id="terminal_id" value="<?php echo $name;?>" readonly>
            <input type="text" name="playlist_id" id="playlist_id" value="<?php echo $playlist_id;?>" readonly>
