...
<div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel"> 
          <div class="x_content">
           ...
           // print_r($tid);exit();
             ...
             $tt1=$this->db->query("SELECT a.`groupt_id` FROM `tbl_playlist_log` a,`tbl_playlist` b, `tbl_group_terminal` c WHERE a.`playlist_id` = b.id AND a.`created_by`=1 AND a.groupt_id =c.id AND a.`playlist_id`=$pid AND a.id=$tid");
        ...
        $playlist2 = $this->db->query("SELECT a.`terminal_id` FROM `tbl_playlist_log` a,`tbl_playlist` b, `tbl_terminal` c WHERE a.`playlist_id` = b.id AND a.`created_by`=1 AND a.terminal_id =c.id AND a.`playlist_id`=$pid AND a.id=$tid");

         $playlist4 = $this->db->query("SELECT a.id as tid, a.terminal_id, a.groupt_id FROM `tbl_playlist_log` a WHERE a.`created_by`=1 AND a.`playlist_id`=$pid AND a.id=$tid AND a.terminal_id=0 AND a.`created_by`=$user_id AND a.groupt_id=0");
...
$playlist1 = $this->db->query("SELECT a.id as tid,c.name, c.id as gid FROM `tbl_playlist_log` a, `tbl_group_terminal` c WHERE a.`created_by`=1 AND a.`playlist_id`=$pid AND a.id=$tid AND c.id IN($grp_terminal) AND a.`created_by`=$user_id"); 
...
//echo "SELECT a.id as tid,c.name, c.id as gid FROM `tbl_playlist_log` a, `tbl_group_terminal` c WHERE a.`created_by`=1 AND a.`playlist_id`=$pid AND a.id=$tid AND c.id IN($grp_terminal) AND a.`created_by`=$user_id";
   ...
   //print_r($terminal1); exit();
     ...
     if (isset($playlist2) && !empty($playlist2)) {
          ...
          $playlist3 = $this->db->query("SELECT a.id as tid,c.name, c.id as gid FROM `tbl_playlist_log` a, `tbl_terminal` c WHERE a.`created_by`=1 AND a.`playlist_id`=$pid AND a.id=$tid AND c.id =$terminal AND a.`created_by`=$user_id");
...
<script>  
 $(document).ready(function(){  
      $('.view_data').click(function(){  
           var id = $(this).attr("id");  
           $.ajax({  
                url:"<?php echo base_url(); ?>playlist_log/get_details",  
                method:"post",  
                data:{id:id},  
 ...
 $(document).ready(function(){  
      $('.view_data1').click(function(){  
           var id = $(this).attr("id");  
           $.ajax({  
                url:"<?php echo base_url(); ?>playlist_log/get_details1",  
                method:"post",  
                data:{id:id},  
                success:function(data){  
                     $('#terminal_detail').html(data);  
