...
<div class="inner_div">
    <?php $layouts = isset($layout->layout) && !empty(unserialize($layout->layout)) ? unserialize($layout->layout) : array();
            if(!empty($layouts)){ $i = 0;
                // foreach ($layouts as $skey => $svalue) {

                //  $i++;

                //  echo "<textarea class='layout-item'>"; 
...
<script>
    ...
    $(document).ready(function(){
        ...
        function showItem(){
            ...
            currentItem =  layoutItems[currentItemIndex];
        ...
        // $( ".layout-item" ).each(function( index ) {
        //     //layout.push(JSON.parse($( this ).text()));
        //     var item = $( this ).text();
       ...
       $(layoutItems).each(function(item){
            switch(this.type){
                case "video":
                plVideos.push(this);
                break;
        ...
        layoutItems= plVideos.concat(plImage, plHtml, plMarquee);
