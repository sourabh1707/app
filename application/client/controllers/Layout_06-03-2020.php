        ...
        if($type=='list'){
            ...
            if(isset($data_array) && !empty($data_array)){
                foreach ($data_array as $d) {
                    ...
                    $row[] = $start;
                    $row[] = $d->name;
                     $row[] = '<a href="'.site_url("layout/preview/".encode_string($d->id)).'"  class="btn btn-success btn-sm">View</a> ';
                 ...
                 //    print_r($terminals); exit();
                    ...
                    if($id!='' && !empty($terminals) && $datetime!=''){
                        ...
                        if($ins_id){
                            ...
                            add_log3($log_data,$uid,0,$id,$start_datetime,$end_datetime,$hourdiff,$terminals);
                        }
                        else{
                            $json['status'] = 201;
                            $json['message'] = $this->db->get_where(TBL_ERROR_CODE,array('code'=>$json['status']))->row()->msg;
                    ...
                    if ($this->form_validation->run()){
                        ...
                        if($ins_id){
                            ...
                            $log_data = array();
                                ...
                                add_log3($log_data,$u_id,0,$id,0,0,0,0);
                        }
                        else{
                            $json['status'] = 205;
                            $json['message'] = $this->db->get_where(TBL_ERROR_CODE,array('code'=>$json['status']))->row()->msg;
