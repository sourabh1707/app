...
class V2 extends V2_Controller {
    ...
    public function schedular() {
        ...
        if(isset($layouts)&&!empty($layouts)){        
            foreach($layouts as $pkey =>$layout){
                ...
                $terminals = isset($layout->terminals) && !empty(unserialize($layout->terminals)) ? unserialize($layout->terminals) : array() ;
                if(!empty($tasks) && !empty($terminals)){
                    foreach ($terminals as $tkey => $tvalue) {
                        ...
                        if(!empty($contents)){
                            $html = $this->load->view('template-layout',array('contents'=>$contents),true);
                            if($html!=''){
                                ...
                                //$upd_id = $this->CRUD->update_data(TBL_SCHEDULE,$upd_table,array('id'=>$layout->id));
                                //}
                            }
                        }
                    }
	...
	$layouts = $this->Model->get_layout_data($from_date,$to_date,$start=false);
		...
		$terminals = isset($layout->terminals) && !empty(unserialize($layout->terminals)) ? unserialize($layout->terminals) : array() ;
		        if(!empty($tasks) && !empty($terminals)){
		            foreach ($terminals as $tkey => $tvalue) {
		                ...
		                $params['terminal_id'] = $tvalue;
				        $api_url = $this->api_url.'set_html';
		                $params['html'] = '<body width="100%" hight="100%" style="background-color:#000000"><br>.</p></body>';
		                $response = $this->terminal_response($api_url,$params);
    				file_put_contents('logs1.txt',$params['terminal_id'],FILE_APPEND);
    				file_put_contents('logs2.txt',$params['html'],FILE_APPEND);
    				file_put_contents('logs3.txt',$api_url,FILE_APPEND);
    				if($response['status']==200){
                                    $status = true;
                                    $upd_table = array();
                                    $upd_table['is_schedule'] = '1';
                                    $upd_table['updated_on'] = date(DB_DATETIME_FORMAT);
                                    $upd_id = $this->CRUD->update_data(TBL_SCHEDULE_L,$upd_table,array('id'=>$layout->id));
    ...
    function terminal_response($api_url,$params){
        if($api_url!='' && !empty($params)){
            ...
            //file_put_contents('logs.txt',$ch,FILE_APPEND);
            $return['status'] = 200;
